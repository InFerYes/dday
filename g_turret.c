...
void turret_breach_think(edict_t* self)
{
	...
	if ((self->move_angles[YAW] < self->pos1[YAW]) || (self->move_angles[YAW] > self->pos2[YAW]))
	{
		...
		dmin = fabsf(self->pos1[YAW] - self->move_angles[YAW]); /* MetalGod use float version of fabs! */
		if (dmin < -180)
			dmin += 360;
		else if (dmin > 180)
			dmin -= 360;
		dmax = fabsf(self->pos2[YAW] - self->move_angles[YAW]); /* MetalGod use float version of fabs! */
		if (dmax < -180)
			dmax += 360;
		else if (dmax > 180)
			dmax -= 360;
		if (fabsf(dmin) < fabsf(dmax)) /* MetalGod use float version of fabs! */
