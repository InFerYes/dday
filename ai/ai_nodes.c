...
qboolean AI_PredictJumpadDestity(edict_t* ent, vec3_t out)
{
	...
	htime = sqrt((tmpfloat));
	vtime = sqrtf((target->s.origin[2] - pad_origin[2])); /* MetalGod use float version of sqrt */
...
qboolean AI_LoadPLKFile(char* mapname)
{
	...
	// check version
	bytes_read = fread(&version, sizeof(int), 1, pIn);
	...
	bytes_read = fread(&nav.num_nodes, sizeof(int), 1, pIn);

	for (i = 0; i < nav.num_nodes; i++)
		bytes_read = fread(&nodes[i], sizeof(nav_node_t), 1, pIn);

	for (i = 0; i < nav.num_nodes; i++)
		bytes_read = fread(&pLinks[i], sizeof(nav_plink_t), 1, pIn);
