...
//	target[1] += skilloffset * (random()-0.5) * ((MAX_BOT_SKILL - self->ai->pers.skillLevel) *2);
	...
	if (self->enemy && self->enemy->client)
	{
		target[0] += skilloffset * 2.0F * (random() - 0.5F); /* MetalGod made these explicitly floats */
		target[1] += skilloffset * 2.0F * (random() - 0.5F); /* MetalGod made these explicitly floats */
	}
