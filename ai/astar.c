...
static int AStar_PLinkDistance(int	n1, int n2) /* MetalGod This used old-style declarator, so added int. */
{
	int	i;
...
static int	Astar_HDist_ManhatanGuess(int node)
{
	...
	for (i = 0; i < 3; i++)
	{
		DistVec[i] = fabsf(nodes[goalNode].origin[i] - nodes[node].origin[i]); /* MetalGod use float version of fabs */
	}
...
static void AStar_PutAdjacentsInOpen(int node) /* MetalGod This used old-style declarator, so added int. */
{
	int	i;
	int rand_ignore = 0;
