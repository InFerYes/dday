...
void AngleMove_Final(edict_t* ent)
{
	...
	VectorScale(move, 1.0F / FRAMETIME, ent->avelocity); /* MetalGod made explicit float */
...
void plat_CalcAcceleratedMove(moveinfo_t* moveinfo)
{
	...
	if ((moveinfo->remaining_distance - accel_dist - decel_dist) < 0)
	{
		...
		moveinfo->move_speed = (-2 + sqrtf(4 - 4 * f * (-2 * moveinfo->remaining_distance))) / (2 * f); /* MetalGod use the float version of sqrt */
...
void plat_Accelerate(moveinfo_t* moveinfo)
{
	...
	if (moveinfo->current_speed < moveinfo->speed)
	{
		...
		// entire move
		p1_distance = moveinfo->remaining_distance - moveinfo->decel_distance;
		p1_speed = (old_speed + moveinfo->move_speed) / 2.0F; /* MetalGod Explicit float */
...
void SP_func_plat(edict_t* ent)
{
	...
	if (!ent->speed)
		...
		ent->speed *= 0.1F; /* MetalGod made explicit float */

	if (!ent->accel)
		ent->accel = 5;
	else
		ent->accel *= 0.1F; /* MetalGod made explicit float */

	if (!ent->decel)
		ent->decel = 5;
	else
		ent->decel *= 0.1F; /* MetalGod made explicit float */
...
void SP_func_door_secret(edict_t* ent)
{
	...
	if (ent->spawnflags & SECRET_1ST_DOWN)
		width = fabsf(DotProduct(up, ent->size)); /* MetalGod use the float version of fabs */
	else
		width = fabsf(DotProduct(right, ent->size)); /* MetalGod use the float version of fabs */
	length = fabsf(DotProduct(forward, ent->size)); /* MetalGod use the float version of fabs */
	if (ent->spawnflags & SECRET_1ST_DOWN)
